...
var Cursor = exports.Cursor = function(query) {
    ...
    self.sort = function(naturalMongoSyntax) {
        ...
        _(naturalMongoSyntax).each(function(orderNum, field) {
            var key = (orderNum === SortDescendingNum) ? SortDescendingKey : SortAscendingKey
            return sort.push([field, key])
        })
        return self
    }
...
var Collection = exports.Collection = function(database, name) {
    ...
    function connection(cb) {
        ...
        return database.connection(function(err, db) {
    ...
    self.ensureIndex = function(index, options, cb) {
        ...
        _(index).each(function(direction, fieldName) {
            return indexArray.push([fieldName, direction])
        })

        return connection(function(err, connection) {
            if (err) return cb(err) 

            return connection.ensureIndex(indexArray, unique, function(err, indexName) {
                connection.done()
                return cb(err, indexName)
            })
        })
    }
    ...
    self.dropIndexes = function(cb) {
        ...
        return connection(function(err, connection) {
            ...
            return connection.dropIndexes(function(err) {
                connection.done()
                return cb(err)
            })
        })
    ...
    self.renameCollection = function(targetName, dropTarget, cb) {
        ...
        function performRename() {
            ...
            connection(function(err, connection) {
                ...
                return connection.rename(database.prefixName(targetName), function(err, collection) {
                    connection.done()
                    return cb(err)
                })
            })
        ...
        if (dropTarget) {
            ...
            return targetCollection.drop(function(err) {
                if (err) return cb(err) 
    ...
    self.insert = function(docs, cb) {
        ...
        return connection(function(err, connection) {
            ...
            return connection.insert(docs, function(err, doc) {
                connection.done()
                return cb(err, doc)
            })
        })
    ...
    self.remove = function(selector, cb) {
        ...
        return connection(function(err, connection) {
            ...
            return connection.remove(selector, function(err) {
                connection.done()
                return cb(err)
            })
        })
    ...
    self.drop = function(cb) {
        ...
        return connection(function(err, connection) {
            ...
            return connection.drop(function(err) {
                connection.done()
                return err && err.message === 'ns not found' ? cb(null, false) : cb(err, true)
            })
        })
    }
    ...
    self.save = function(doc, cb) {
        ...
        return connection(function(err, connection) {
            ...
            return connection.save(doc, function(err, doc) {
                connection.done()
                return cb(err, doc)
            })
        })
    ...
    self.update = function(selector, updates, upsert, multi, cb) {
        ...
        return connection(function(err, connection) {
            ...
            return connection.update(selector, updates, options, function(err, doc) {
                connection.done()
                return cb(err, doc)
            })
        })
        ...
        return new Cursor(function(callback) {
            ...
            return connection(function(err, connection) {
                ...
                if (fields) {
                    ...
                    options.fields = _(fields).map(function(num, fieldName) {
                        return fieldName
                    })
                }

                return connection.find(selector, options, function(err, cursor) {
...
var Database = exports.Database = function(host, port, name, prefix) {
    ...
    self.lastError = function(cb) {
        ...
        return self.connection(function(err, connection) {
            if (err) return cb(err) 

            return connection.lastError(function(err, lastError) {
                connection.done()
                return cb(err, lastError)
            })
        })
    }
    ...
    self.dropAllCollections = function(cb) {
        ...
        return _(self).each(function(obj, name) {
        
            if (obj instanceof Collection) {
                waitingForDrop++
        
                return obj.drop(function(err) {
                    waitingForDrop--
                    if (err) return cb(err) 
                    return waitingForDrop === 0 ? cb() : null
                })
            }
        })
    }
    ...
    if (this.shouldAutoClose && this.started === this.completed) {
        ...
        return process.nextTick(function() {
            ...
            if (self.started === current) {
                ...
                return (self.autoCloser = setTimeout(later, exports.AutoCloseTimeout))
            }
        })
    }
}
